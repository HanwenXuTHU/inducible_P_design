...
import torch
from sequenceModels import cal_gradient_penalty

class P2P_model(BaseModel):

    def __init__(self, opt, sequence_length=50):
        ...
        self.netG = sequenceModels.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, gpu_ids=opt.gpu_ids, sequence_length=sequence_length)
    ...
    def backward_D(self):
        ...
        if self.gan_mode == 'wgangp':
            gp_loss = cal_gradient_penalty(self.netD, realAB, fakeAB.detach(), self.device)
            self.loss_D += gp_loss[0]
